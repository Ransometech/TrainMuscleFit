...
def index():
    ...
    portfolio = db.execute("""
        SELECT symbol, SUM(shares) AS shares, ROUND(price, 2), ROUND(SUM(total),2) AS total
        FROM portfolio
        WHERE user_id = ?
        GROUP BY symbol
    """, user_id)
...
def history():
    ...
    history = db.execute("SELECT symbol, shares, ROUND(price, 2), ROUND(total, 2), timestamp FROM portfolio WHERE user_id = ?", user_id)
